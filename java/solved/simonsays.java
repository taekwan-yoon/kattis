...
public class simonsays {
    public static void main(String[] args) {
        ...
        scanner.nextLine();
        List<String> valid_instructions = new ArrayList<>();

        for (int i=0; i<num; i++){
            String instruction = scanner.nextLine();
            if (instruction.contains("Simon says")) {
                valid_instructions.add(instruction);
            }
        }

        for (String instruction : valid_instructions) {
            System.out.println(instruction.substring(10, instruction.length()));
        }
