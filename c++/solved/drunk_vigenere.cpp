...
int main() {
    ...
    string encoded, decrypt;
    cin >> encoded >> decrypt;
    char decrypted[encoded.size()];
    int result;
    char letter;

    for (int i=0; i < encoded.size(); i++) {
        if (i % 2 == 0) {
            result = int(encoded[i]) - 65 - int(decrypt[i]) + 65;
            if (result < 0) {
                result += 26;
            }
        } else {
